...
function NightscoutClient(adapter, URL, secretHash) {
    ...
    this.nsSocket.on('alarm', data => {
        adapter.log.debug('[CLIENT] alarm: ' + data);
        return adapter.setState('data.alarm', data && data.level && data.level === 1, true);
    });

    this.nsSocket.on('urgent_alarm', data => {
        adapter.log.debug('[CLIENT] urgentAlarm: ' + data);
        return adapter.setState('data.urgentAlarm', data && data.level && data.level === 3, true);
    });
