...
function getLatestTreatmentsInfo(treatments) {
    const sensorInfo = {
        found: false,
        age: 0,
        days: 0,
        ...
        try {
            ...
            if (dataUpdate) {
                ...
                if (dataUpdate.treatments && dataUpdate.treatments.length) {
                    ...
                    const cannulaInfo = getLatestTreatmentsInfo(sitechangeTreatments);
                    ...
                    const sensorInfo = getLatestTreatmentsInfo(sensorTreatments);
                    if (sensorInfo.found) {
                ...
                if (!siteChangeUpdated) {
                    const treatmentDate = adapter.getState('data.cage.changed').val || -1;
                    if (treatmentDate >= 0) {
                        const b = moment(treatmentDate);
                        const days = a.diff(b, 'days');
                        const hours = a.diff(b, 'hours') - days * 24;
                        ...
                        adapter.setState('data.cage.age', {ts: now, ack: true, val: age});
                        adapter.setState('data.cage.days', {ts: now, ack: true, val: days});
                        adapter.setState('data.cage.hours', {ts: now, ack: true, val: hours});
                    }
                }
                if (!sensorUpdated) {
                    const treatmentDate = adapter.getState('data.sage.changed').val || -1;
                    if (treatmentDate >= 0) {
                        const b = moment(treatmentDate);
                        const days = a.diff(b, 'days');
                        const hours = a.diff(b, 'hours') - days * 24;
                        ...
                        adapter.setState('data.sage.age', {ts: now, ack: true, val: age});
                        adapter.setState('data.sage.days', {ts: now, ack: true, val: days});
                        adapter.setState('data.sage.hours', {ts: now, ack: true, val: hours});
                    }
                }
            }
        } catch (error) {
